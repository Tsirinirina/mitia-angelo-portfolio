...
import PreviewLinkImage from "@/components/preview-link-image/preview-link-image";
  ...
  return (
    <motion.div
      ...
      className={`${size === "MD" ? styles.md : styles.lg} ${
        styles.container
      } ${noImage ? styles.hMd : styles.hLg} `}
      id={`projet-${title}`}
    >
      <div
        className={`${styles.status} ${status === "DEV" && styles.dev} ${
          status === "PROD" && styles.prod
        } ${status === "MAINTENANCE" && styles.maintain}`}
      >
        {status}
      </div>
      {!noImage && (
      ...
      <div className={styles.content}>
        <div className={styles.work}>
          ...
          <div className={styles.info}>
            ...
            <div className={`inconsolate ${styles.date}`}>
              {new DateFormat(publishDate as string).toSimpleDate()}
            </div>
          </div>
        </div>
