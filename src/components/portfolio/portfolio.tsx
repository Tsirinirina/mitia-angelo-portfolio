...
import { GetAllExperience } from "@/services/experience/experience.service";
import { GetAllProject } from "@/services/project/project.service";
import { UrlType } from "@/services/url/url";
import { GetAllUrl } from "@/services/url/url.service";
...
export default function Portfolio() {
  ...
  const getAllData = async () => {
    try {
      ...
      const skillsData: Skill[] = await GetAllSkill();

      setSkills(skillsData.filter((item) => item.category !== "DATABASE"));
      const projectsData = await GetAllProject();
      setProjects(projectsData);
      const languageData = await GetAllLanguage();
      setLanguages(languageData);
      const experiencesData = await GetAllExperience();
      setExperiences(experiencesData);
      const urlsData = await GetAllUrl();
      setUrls(urlsData);
      if (
        personalData &&
        certificateData &&
      ...
      {!isLoading && (
        <>
          ...
          <main className={styles.main}>
            ...
            <Espacer size="lg" />
            <ExperienceContainer data={experiences} />
            <Espacer size="lg" />
          </main>
          <InfoContainer personal={personal} urls={urls} />
        </>
      )}
    </div>
  );
